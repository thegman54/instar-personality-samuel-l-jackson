"""
Personality Samuel L. Jackson — Admin API Handlers
...
TABLE_PREFIX = "personality_samuel_l_jackson"
...
async def list_traits(pool, slug, **_):
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            ...
            FROM personality_samuel_l_jackson_traits
    ...
    async with pool.acquire() as conn:
        try:
            row = await conn.fetchrow(
                """
                INSERT INTO personality_samuel_l_jackson_traits
...
async def delete_trait(pool, slug, item_id=None, **_):
    async with pool.acquire() as conn:
        result = await conn.execute(
            "DELETE FROM personality_samuel_l_jackson_traits WHERE id = $1::uuid AND profile_slug = $2",
            item_id, slug,
        )
    if result == "DELETE 0":
...
async def list_quotes(pool, slug, **_):
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            ...
            FROM personality_samuel_l_jackson_quotes
            WHERE profile_slug = $1
...
async def create_quote(pool, slug, body=None, **_):
    ...
    async with pool.acquire() as conn:
        try:
            row = await conn.fetchrow(
                """
                INSERT INTO personality_samuel_l_jackson_quotes
...
async def delete_quote(pool, slug, item_id=None, **_):
    async with pool.acquire() as conn:
        result = await conn.execute(
            "DELETE FROM personality_samuel_l_jackson_quotes WHERE id = $1::uuid AND profile_slug = $2",
            item_id, slug,
        )
    if result == "DELETE 0":
...
async def list_lexicon(pool, slug, **_):
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            ...
            FROM personality_samuel_l_jackson_lexicon
            WHERE profile_slug = $1
...
async def create_lexicon(pool, slug, body=None, **_):
    ...
    async with pool.acquire() as conn:
        try:
            row = await conn.fetchrow(
                """
                INSERT INTO personality_samuel_l_jackson_lexicon
...
async def delete_lexicon(pool, slug, item_id=None, **_):
    async with pool.acquire() as conn:
        result = await conn.execute(
            "DELETE FROM personality_samuel_l_jackson_lexicon WHERE id = $1::uuid AND profile_slug = $2",
            item_id, slug,
        )
    if result == "DELETE 0":
...
async def list_reactions(pool, slug, **_):
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            ...
            FROM personality_samuel_l_jackson_reactions
            WHERE profile_slug = $1
...
async def create_reaction(pool, slug, body=None, **_):
    ...
    async with pool.acquire() as conn:
        try:
            row = await conn.fetchrow(
                """
                INSERT INTO personality_samuel_l_jackson_reactions
...
async def delete_reaction(pool, slug, item_id=None, **_):
    async with pool.acquire() as conn:
        result = await conn.execute(
            "DELETE FROM personality_samuel_l_jackson_reactions WHERE id = $1::uuid AND profile_slug = $2",
            item_id, slug,
        )
    if result == "DELETE 0":
...
#
# Example: GET /skill_api/personality_samuel_l_jackson/hal_test/traits
