-- Personality Samuel L. Jackson: Lexicon table
-- Vocabulary fingerprint — words/phrases to favor, avoid, or use situationally.

CREATE TABLE IF NOT EXISTS personality_samuel_l_jackson_lexicon (
...
CREATE INDEX IF NOT EXISTS idx_pslj_lexicon_profile
    ON personality_samuel_l_jackson_lexicon(profile_slug);
CREATE INDEX IF NOT EXISTS idx_pslj_lexicon_type
    ON personality_samuel_l_jackson_lexicon(profile_slug, usage_type);
CREATE INDEX IF NOT EXISTS idx_pslj_lexicon_freq
    ON personality_samuel_l_jackson_lexicon(profile_slug, frequency);
