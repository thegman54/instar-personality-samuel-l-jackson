-- Personality Samuel L. Jackson: Reactions table
-- Trigger → response pattern mappings for contextual behavior.

CREATE TABLE IF NOT EXISTS personality_samuel_l_jackson_reactions (
...
CREATE INDEX IF NOT EXISTS idx_pslj_reactions_profile
    ON personality_samuel_l_jackson_reactions(profile_slug);
CREATE INDEX IF NOT EXISTS idx_pslj_reactions_trigger
    ON personality_samuel_l_jackson_reactions(profile_slug, trigger_type);
CREATE INDEX IF NOT EXISTS idx_pslj_reactions_intensity
    ON personality_samuel_l_jackson_reactions(profile_slug, intensity);
CREATE INDEX IF NOT EXISTS idx_pslj_reactions_tags
    ON personality_samuel_l_jackson_reactions USING GIN(tags);
